...
void syslog(const char *message) {
  if (Settings.Syslog_IP[0] != 0) {
    IPAddress broadcastIP(Settings.Syslog_IP[0], Settings.Syslog_IP[1],
                          Settings.Syslog_IP[2], Settings.Syslog_IP[3]);
    portUDP.beginPacket(broadcastIP, 514);
    char str[256];
    str[0] = 0;
    snprintf_P(str, sizeof(str), PSTR("<7>ESP Unit: %u : %s"), Settings.Unit,
               message);
    portUDP.write(str);
    portUDP.endPacket();
  }
  ...
  if (packetSize) {
    ...
    if (packetBuffer[0] != 255) {
      ...
      case 1: // sysinfo message
      {
        ...
        char macaddress[20];
        sprintf_P(macaddress, PSTR("%02x:%02x:%02x:%02x:%02x:%02x"), mac[0],
      ...
      case 3: // sensor info
      {
        if (Settings.GlobalSync) {
          ...
          // to prevent flash wear out (bugs in communication?) we can only
          // write to an empty task
          // so it will write only once and has to be cleared manually through
          // webgui
          if (Settings.TaskDeviceNumber[infoReply.destTaskIndex] == 0) {
            Settings.TaskDeviceNumber[infoReply.destTaskIndex] =
                infoReply.deviceNumber;
      ...
      case 5: // sensor data
      {
        if (Settings.GlobalSync) {
          ...
          if (Settings.TaskDeviceDataFeed[dataReply.destTaskIndex] != 0) {
            for (byte x = 0; x < VARS_PER_TASK; x++) {
              UserVar[dataReply.destTaskIndex * VARS_PER_TASK + x] =
                  dataReply.Values[x];
            }
...
void SendUDPTaskData(byte destUnit, byte sourceTaskIndex, byte destTaskIndex) {
  ...
  for (byte x = 0; x < VARS_PER_TASK; x++)
    dataReply.Values[x] =
        UserVar[dataReply.sourceTaskIndex * VARS_PER_TASK + x];
...
void sendUDP(byte unit, byte *data, byte size) {
  ...
  IPAddress remoteNodeIP(Nodes[unit].ip[0], Nodes[unit].ip[1],
                         Nodes[unit].ip[2], Nodes[unit].ip[3]);
...
void refreshNodeList() {
  for (byte counter = 0; counter < UNIT_MAX; counter++) {
    if (Nodes[counter].ip[0] != 0) {
      ...
      if (Nodes[counter].age >
          10) // if entry to old, clear this node ip from the list.
        for (byte x = 0; x < 4; x++)
...
void SSDP_schema(WiFiClient client) {
  ...
  String ssdp_schema = F("HTTP/1.1 200 OK\r\n"
...
void SSDP_send(byte method) {
  ...
  String _ssdp_response_template = F("HTTP/1.1 200 OK\r\n"
                                     "EXT:\r\n"
                                     "ST: upnp:rootdevice\r\n");
  ...
  String _ssdp_packet_template =
      F("%s" // _ssdp_response_template / _ssdp_notify_template
  ...
  int len = snprintf(buffer, sizeof(buffer), _ssdp_packet_template.c_str(),
                     (method == 0) ? _ssdp_response_template.c_str()
                                   : _ssdp_notify_template.c_str(),
                     SSDP_INTERVAL, Settings.Build, uuid, IP2STR(&ip));
...
void SSDP_update() {
  ...
  if (!_pending && _server->next()) {
    ...
    while (_server->getSize() > 0) {
      ...
      case METHOD:
        if (c == ' ') {
          if (strcmp_P(buffer, PSTR("M-SEARCH")) == 0)
            method = SEARCH;
          else if (strcmp_P(buffer, PSTR("NOTIFY")) == 0)
            method = NOTIFY;
      ...
      case VALUE:
        if (cr == 2) {
          ...
          case ST:
            ...
            // ABORT
            if (strcmp_P(buffer,
                         PSTR("urn:schemas-upnp-org:device:BinaryLight:1")) ==
                0) {
              _pending = true;
              _process_time = millis();
              state = KEY;
  ...
  if (_pending && (millis() - _process_time) > _delay) {
    ...
    _delay = 0;
    SSDP_send(NONE);
  } else if (_notify_time == 0 ||
             (millis() - _notify_time) > (SSDP_INTERVAL * 1000L)) {
    _notify_time = millis();
    SSDP_send(NOTIFY);
  }
